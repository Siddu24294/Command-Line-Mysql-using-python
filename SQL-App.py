...
def Display_Table(table,fields='*'):
	fields_str=fields
	...
	display_table = 'select '+fields_str+' from ' + table
	keys = tuple(d.keys())
	du.execute(display_table)
	records = du.fetchall() + [keys]
	ll = []
	...
	for i in range(len(d)):
		length = max(list(len(str(int[i])) for int in records))
		ll.append(length)

	for i in range(len(keys)):print('||', ' ' * (ll[i] - len(keys[i])) + keys[i], end=' ')
	print('||')
	for i in records[:-1]:
		for int in range(len(i)): print('||', ' ' * (ll[int] - len(str(i[int]))) + str(i[int]), end=' ')
		print('||')
...
░░ II     -> Insert Values                                        ░░
░░ III    -> delete values                                        ░░
░░ IV     -> displaying a selected view from a given table        ░░
░░ V      -> using special commands such as group by,where as etc ░░
░░ VI     -> Exiting to previous menu.                            ░░
░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
'''

table_creation_instructions='''
* INSTRUCTIONS FOR TABLE CREATION *
dOne field must have a unique value for each record.Such a field is called primary key.
type y to make a desired field primary key.One table can have only 1 primary key.
type n to not to make it a primary key.
eg->Adm_no.,employee_id etc
░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
...
# Table Manipulation
		...
		while 1:
			m_choice=input('enter a choice from manipulation table(I-V):')

			if m_choice=='I':
				print('You have chosen to change the table for manipulation.','\n')
				table=input('Enter table NAme:')
				Display_Table(table)

			elif m_choice=='II':
				print('you have chosen to insert values.')

			elif m_choice=='IV':
				print('\nYou have chosen to display selected view of a table.')
				print('Enter field names (Case-Sensitive) separated by a comma(,).')
				print('Refer to the above displayed table to get the field names.')
				print('input eg->in a table with fields Name,id,BloodGrp,Grade,Marks input should be given as:')
				print('              Enter field names:Name,BloodGrp,Marks')
				print('          for selecting the fields Name ,bloodGrp, Marks.\n')
				fields=input('Enter filed names:')
				print('\nselective view of fields',fields,'ffrom table',table,'is:')
				Display_Table(table,fields)

			elif m_choice=='VI':print('exiting to previous menu...\n');print(menu);break
...
def connect():
	...
	try:
		connection_object = msc.connect(host=host_name, user=user_name, passwd=password, database=data_name);factor = 1
	except msc.errors.ProgrammingError:
		factor = -1;print('ERROR due to wrong data.')

	if factor == -1:
		print('Please fill correct data.To exit type "-/exit" instead of data.');return connect()
	else:
		print('Connection Successful ☻');return connection_object
...
def field_input(key_list=prim_key):
	...
	x = ''
	if ' ' in field_name: print('Rewrite field name replacing space with underscores(_)');return field_input()
	if type not in list(data_type_dict.keys()):
		print('Write symbol of data type as given in INSTRUCTIONS FOR TABLE CREATION .');return field_input()
	else:
		if key.lower() == 'y':
			key_list = ['y'];x = 'primary key'
		else:
			x = ''
		return field_name, type, x
...
while 1:
	...
	# Tabke creation
		...
		create_table += field_list[-1] + ')'
		try:
			du.execute(create_table);dbu.commit();print('table created successfully...')
		except msc.errors.ProgrammingError:
	...
	# Table deletion
		...
		if confirm.lower() == 'y':
			...
			print('please try again if you want to delete ' + table + '.')

	elif int(choice) == 5:
		print(menu)

	elif int(choice) == 6:
		print('Thanks for using this utility.');print('Exiting...');quit();break"""